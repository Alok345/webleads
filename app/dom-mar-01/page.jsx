...
export default function NRI1502() {
  ...
  useEffect(() => {
    const collectionName = "dom-marathi-01";
  ...
  const handlePushLead = async (leadId) => {
    try {
      ...
      const leadRef = doc(db, "dom-marathi-01", leadId);
  ...
  const handleDownloadExcel = () => {
    ...
    const filename = `dom-marathi-01-Leads-${new Date().toISOString().split("T")[0]}.xlsx`;
  ...
  const handleMarkAsJunk = async (leadId) => {
    try {
      ...
      await updateDoc(doc(db, "dom-marathi-01", leadId), {
        status: "junk",
          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div className="bg-white rounded-xl shadow-sm p-6 mb-6">
              <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
                <div>
                  <h1 className="text-3xl font-bold text-gray-900">Domestic Marathi Leads</h1>
                  ...
                  <tbody className="divide-y divide-gray-200">
                    <AnimatePresence>
                      {currentItems.map((lead) =>{ 
                        const actionDate = lead.pushedAt || lead.duplicateMarkedAt || lead.junkAt;
                        return (
                        <motion.tr
                          key={lead.id}
